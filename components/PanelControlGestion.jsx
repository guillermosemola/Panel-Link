...
.pcg-tbl tbody tr.clickable{cursor:pointer}
.pcg-tbl tbody tr.clickable:hover{background:rgba(94,234,212,.04)}
...
.pcg-edit-hint{font-size:10px;color:var(--fnt);text-align:right;margin-bottom:8px}
`

export default function PanelControlGestion({ proyecto, controlPresupuestario, performanceComercial, descalceFinanciero, margenTeoricoVsReal, unidades=[], transacciones=[], puedeEditar=false, empresaId, onRefresh }) {
    ...
    <div className="pcg">
      ...
      {tab==='inventario' && (
        <>
          ...
          {puedeEditar && <div className="pcg-edit-hint">Clic en una unidad para editar asignación o registrar venta →</div>}
            ...
            <table className="pcg-tbl">
              ...
              <tbody>
                ...
                {uFiltradas.map(u=>{
                  ...
                  return (
                    <tr key={u.id} className={puedeEditar?'clickable':''} onClick={()=>handleUnidadClick(u)}>
