...
function NuevoProyectoInner() {
  ...
  async function confirmarYGuardar() {
    ...
    if (proyExistente) {
      ...
      setMsg({ok:true,text:`✅ EV cargado en "${nombre}". Redirigiendo...`})
      ...
      await supabase.from('variables_proyecto').insert({
        proyecto_id: newProy.id, modificado_por: perfil.id, sector_origen: 'finanzas',
        m2_totales: P.m2, plazo_meses: P.plazo, costo_directo_m2: P.construccion,
        precio_mercado_m2: P.pfondeo,
      }).catch(()=>{})
      setMsg({ok:true,text:`✅ Proyecto "${nombre}" creado. Redirigiendo...`})
