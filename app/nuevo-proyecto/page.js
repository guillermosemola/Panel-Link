...
export default function NuevoProyectoPage() {
  ...
  const proyectoId   = searchParams.get('id') // si viene de un proyecto existente sin EV

  const [step,    setStep]    = useState(1)  // 1=datos básicos, 2=EV, 3=confirmación
  ...
  useEffect(() => {
    async function load() {
      ...
      // Si viene con ID de proyecto existente, cargar datos
      if (proyectoId) {
  ...
  async function confirmarYGuardar() {
    ...
    const evSnapshot = JSON.stringify({
      inputs: P,
      resultado: {
        c_total_impl: R.c_total_impl,
        costo_total_impl: R.costo_total_impl,
        escenarios: R.escenarios,
        rubros: R.rubros,
        tir_a: R.tir_a,
        metros_x_cupo: R.metros_x_cupo,
        valor_cupo: R.valor_cupo,
        m2_fondeo: R.m2_fondeo,
        precio_equilibrio: R.precio_equilibrio,
      }
    })
    ...
    if (proyExistente) {
      // Actualizar proyecto existente con el EV
      ...
      setSaving(false)
      setMsg({ok:true,text:`✅ EV cargado en "${nombre}". Redirigiendo al panel...`})
      setTimeout(() => router.push(`/proyecto/${proyExistente.id}/gestion`), 1200)
    } else {
      ...
      await supabase.from('variables_proyecto').insert({
        proyecto_id:      newProy.id,
        modificado_por:   perfil.id,
        sector_origen:    'finanzas',
        m2_totales:       P.m2,
        plazo_meses:      P.plazo,
        costo_directo_m2: P.construccion,
        precio_mercado_m2:P.pfondeo,
      }).catch(()=>{})

      setSaving(false)
      setMsg({ok:true,text:`✅ Proyecto "${nombre}" creado con EV. Redirigiendo...`})
      setTimeout(() => router.push(`/proyecto/${newProy.id}/gestion`), 1200)
    }
  }
  ...
  return (
    <div style={{minHeight:'100vh',background:'#f8f8f6',fontFamily:'system-ui,sans-serif'}}>
      ...
      <nav style={{background:'#fff',borderBottom:'0.5px solid #e0ddd6',padding:'0 1.5rem',
        ...
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <span style={{fontSize:14,fontWeight:500}}>
            {proyExistente ? `Cargar EV — ${proyExistente.nombre}` : 'Nuevo proyecto'}
          </span>
        </div>
      ...
      {/* Progress steps */}
      <div style={{background:'#fff',borderBottom:'0.5px solid #e0ddd6',padding:'12px 2rem',
        display:'flex',alignItems:'center',gap:0}}>
        {[
          {n:1, label: proyExistente ? 'Proyecto existente' : 'Datos del proyecto'},
          {n:2, label:'Evaluación de Viabilidad (EV)'},
          {n:3, label:'Confirmar y crear'},
        ].map((s,i)=>(
          <div key={s.n} style={{display:'flex',alignItems:'center'}}>
            <div style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer'}}
              onClick={()=>{ if(s.n < step || (s.n===1&&!proyExistente)) setStep(s.n) }}>
              <div style={{width:28,height:28,borderRadius:'50%',display:'flex',
                alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:600,
                background: step===s.n ? '#1a1a18' : step>s.n ? '#4A9E3F' : '#f0ede8',
                color: step>=s.n ? '#fff' : '#aaa'}}>
                {step>s.n ? '✓' : s.n}
              </div>
              <span style={{fontSize:13,color:step===s.n?'#1a1a18':step>s.n?'#4A9E3F':'#aaa',
        ...
        <div style={{maxWidth:560,margin:'3rem auto',padding:'0 1.5rem'}}>
          <div style={{background:'#fff',borderRadius:14,padding:'2rem',border:'0.5px solid #e0ddd6'}}>
            ...
            <p style={{fontSize:13,color:'#888',marginBottom:24}}>
              Completá la información básica. El EV se configurará en el siguiente paso.
            </p>
            ...
            <div style={{marginBottom:16}}>
              ...
              <input value={nombre} onChange={e=>setNombre(e.target.value)}
                placeholder="Ej: Neuquén, Marcos Paz..."
                style={{width:'100%',padding:'10px 14px',fontSize:15,fontWeight:500,
                  borderRadius:10,border:'0.5px solid #ccc',outline:'none',
            ...
            <div style={{marginBottom:16}}>
              ...
              <input value={ubicacion} onChange={e=>setUbicacion(e.target.value)}
                placeholder="Ej: Neuquén Capital, Barrio Centro"
                ...
                {[['planificacion','📐 Planificación'],['en_obra','🏗 En obra']].map(([val,label])=>(
                  <button key={val} onClick={()=>setEstado(val)}
                    style={{padding:'10px',borderRadius:8,fontSize:13,cursor:'pointer',
                      ...
                      color: estado===val ? '#fff' : '#666',fontWeight:estado===val?600:400}}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            <button onClick={()=>{ if(!nombre.trim()){return} setStep(2) }}
              disabled={!nombre.trim()}
              style={{width:'100%',padding:'13px',fontSize:14,fontWeight:600,borderRadius:10,
                border:'none',background:nombre.trim()?'#1a1a18':'#e0ddd6',
                color:nombre.trim()?'#fff':'#aaa',cursor:nombre.trim()?'pointer':'not-allowed'}}>
      ...
      {step === 2 && R && (
        <div style={{display:'grid',gridTemplateColumns:'340px 1fr',minHeight:'calc(100vh - 110px)'}}>
          ...
          {/* INPUTS */}
          <div style={{borderRight:'0.5px solid #e0ddd6',background:'#fff',
            padding:'20px',overflowY:'auto',maxHeight:'calc(100vh - 110px)'}}>
              ...
              {Object.keys(PRESETS).map(name=>(
                <button key={name} onClick={()=>loadPreset(name)}
                  style={{padding:'4px 12px',borderRadius:20,fontSize:12,cursor:'pointer',
                    background:preset===name?'#1a1a18':'#fafafa',
                    color:preset===name?'#fff':'#666',
            ...
            <Field label="m² totales" val={`${fmt(P.m2)} m²`}>
              <input type="range" style={sRange} min={500} max={25000} step={50} value={P.m2} onChange={e=>setVal('m2',+e.target.value)}/>
            </Field>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
              <div><div style={{fontSize:11,color:'#888',marginBottom:4}}>% Cocheras</div>
                <input type="number" style={sMini} value={P.pct_coch} min={0} max={60} onChange={e=>setVal('pct_coch',+e.target.value)}/></div>
              <div><div style={{fontSize:11,color:'#888',marginBottom:4}}>Ud. prom (m²)</div>
                <input type="number" style={sMini} value={P.ud_prom} min={20} max={150} onChange={e=>setVal('ud_prom',+e.target.value)}/></div>
            </div>
            <Field label="Plazo de obra" val={`${P.plazo} meses`}>
              <input type="range" style={sRange} min={12} max={72} step={1} value={P.plazo} onChange={e=>setVal('plazo',+e.target.value)}/>
            </Field>

            <span style={sLabel}>Costos (USD/m²)</span>
            <Field label="Construcción" val={`$${P.construccion}`}>
              <input type="range" style={sRange} min={200} max={800} step={5} value={P.construccion} onChange={e=>setVal('construccion',+e.target.value)}/>
            </Field>
            <Field label="Terreno + Infraestructura" val={`$${P.terreno}`}>
              <input type="range" style={sRange} min={0} max={600} step={5} value={P.terreno} onChange={e=>setVal('terreno',+e.target.value)}/>
            </Field>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',
              padding:'8px 0',borderBottom:'0.5px solid #f0ede8',fontSize:13,color:'#555',marginBottom:8}}>
              ...
              <label style={{position:'relative',width:34,height:18,cursor:'pointer'}}>
                ...
                <div style={{position:'absolute',inset:0,borderRadius:10,
                  background:P.canje?'#1a1a18':'#e0ddd6',transition:'background .2s'}}>
                  <div style={{position:'absolute',top:2,left:P.canje?18:2,width:14,height:14,
            ...
            {[
              ...
              <Field key={f.key} label={f.label} val={f.fv(P[f.key])}>
                <input type="range" style={sRange} min={f.min} max={f.max} step={f.step} value={P[f.key]} onChange={e=>setVal(f.key,+e.target.value)}/>
              </Field>
            ))}

            <span style={sLabel}>Precios de venta</span>
            <Field label="Precio de fondeo (USD/m²)" val={`$${fmt(P.pfondeo)}`}>
              <input type="range" style={sRange} min={500} max={2500} step={25} value={P.pfondeo} onChange={e=>setVal('pfondeo',+e.target.value)}/>
            </Field>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
              <div><div style={{fontSize:11,color:'#888',marginBottom:4}}>Prop. contado</div>
                <select style={sMini} value={P.pct_contado} onChange={e=>setVal('pct_contado',+e.target.value)}>
                  <option value={70}>70% contado</option><option value={60}>60% contado</option>
                </select></div>
              <div><div style={{fontSize:11,color:'#888',marginBottom:4}}>Cuotas financiado</div>
                <input type="number" style={sMini} value={P.cuotas} min={6} max={60} onChange={e=>setVal('cuotas',+e.target.value)}/></div>
            </div>
            {[{key:'p1',label:'Precio pesimista'},{key:'p2',label:'Precio esperado'},{key:'p3',label:'Precio optimista'}].map(f=>(
              <Field key={f.key} label={`${f.label} (USD/m²)`} val={`$${fmt(P[f.key])}`}>
                <input type="range" style={sRange} min={600} max={2500} step={25} value={P[f.key]} onChange={e=>setVal(f.key,+e.target.value)}/>
              </Field>
            ))}

            <span style={sLabel}>Modelo inversor</span>
            <Field label="% metros a fondear" val={`${P.pct_fondeo.toFixed(1)}%`}>
              <input type="range" style={sRange} min={0} max={80} step={.5} value={P.pct_fondeo} onChange={e=>setVal('pct_fondeo',+e.target.value)}/>
            </Field>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:24}}>
              <div><div style={{fontSize:11,color:'#888',marginBottom:4}}>Cuotas de obra</div>
                <input type="number" style={sMini} value={P.cuotas_obra} min={6} max={48} onChange={e=>setVal('cuotas_obra',+e.target.value)}/></div>
              <div><div style={{fontSize:11,color:'#888',marginBottom:4}}>N° de cupos</div>
                <input type="number" style={sMini} value={P.cupos} min={1} max={20} onChange={e=>setVal('cupos',+e.target.value)}/></div>
            </div>
          ...
          <div style={{padding:'24px 28px',overflowY:'auto',maxHeight:'calc(100vh - 110px)'}}>
            {/* Alerta viabilidad */}
            <div style={{borderRadius:8,padding:'10px 14px',fontSize:12,marginBottom:20,
              ...(R.viable
                ? {background:'#EAF3DE',border:'0.5px solid #C0DD97',color:'#27500A'}
                : {background:'#FCEBEB',border:'0.5px solid #F09595',color:'#7A1A1A'})}}>
              {R.viable
                ? `✅ Proyecto viable. Margen de fondeo: ${(R.margen_fondeo*100).toFixed(1)}%`
                : `⚠️ Costo ($${fmt(Math.round(R.c_total_impl))}/m²) supera precio de fondeo ($${fmt(P.pfondeo)}/m²).`}
            </div>
              ...
              {['Pesimista','Esperado','Optimista'].map((lbl,i)=>{
                ...
                return (
                  <div key={i} onClick={()=>setEsc(i)}
                    style={{background:escSel===i?'#fafaf8':'#fff',
                      border:`0.5px solid ${escSel===i?'#1a1a18':'#e0ddd6'}`,
            ...
            <div style={{background:'#fff',border:'0.5px solid #e0ddd6',borderRadius:10,padding:'18px'}}>
              <div style={{fontSize:10,fontWeight:600,letterSpacing:'.1em',textTransform:'uppercase',color:'#aaa',marginBottom:14}}>
                Desglose de costos — fideicomiso al costo
              </div>
              <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
                <thead><tr>
                  {['Rubro','USD/m²','Total','%'].map(h=>(
                    <th key={h} style={{fontSize:10,fontWeight:600,letterSpacing:'.08em',textTransform:'uppercase',
                      color:'#aaa',textAlign:h==='Rubro'?'left':'right',padding:'5px 0',
                      borderBottom:'0.5px solid #e0ddd6'}}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {R.rubros.map((r,i)=>(
                    <tr key={i}>
                      ...
                      <td style={{textAlign:'right',padding:'7px 0',borderBottom:'0.5px solid #f5f3ef'}}>
                        <div style={{display:'flex',alignItems:'center',justifyContent:'flex-end',gap:6}}>
                          <div style={{background:'#f0ede8',borderRadius:3,height:4,width:60}}>
                            <div style={{width:`${(r.valor/R.c_total_impl*100).toFixed(1)}%`,height:'100%',borderRadius:3,background:'#1a1a18'}}/>
                          </div>
      ...
      {step === 3 && R && (
        <div style={{maxWidth:640,margin:'3rem auto',padding:'0 1.5rem'}}>
          <div style={{background:'#fff',borderRadius:14,padding:'2rem',border:'0.5px solid #e0ddd6'}}>
            ...
            <p style={{fontSize:13,color:'#888',marginBottom:24}}>
              El EV quedará guardado como <strong>presupuesto base inmutable</strong>. Podés seguir ajustando variables después, pero este snapshot es la línea de base del proyecto.
            </p>

            {/* Resumen */}
            <div style={{background:'#f8f8f6',borderRadius:10,padding:'1.25rem',marginBottom:20,
              border:'0.5px solid #e0ddd6'}}>
              <div style={{fontSize:12,fontWeight:600,color:'#888',marginBottom:12,
                textTransform:'uppercase',letterSpacing:'.08em'}}>Resumen del proyecto</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px 24px'}}>
                {[
                  ['Nombre', nombre],
                  ['Ubicación', ubicacion||'—'],
                  ['Estado', estado],
                  ['m² totales', `${fmt(P.m2)} m²`],
                  ['Plazo', `${P.plazo} meses`],
                  ...
                  ['Escenario seleccionado', ['Pesimista','Esperado','Optimista'][escSel]],
                  ...
                  <div key={l}>
                    ...
                    <div style={{fontSize:13,fontWeight:500,color:'#1a1a18',fontFamily:l.includes('$')||l.includes('m²')||l.includes('%')?'monospace':'inherit'}}>{v}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {msg && (
              <div style={{fontSize:12,padding:'10px 14px',borderRadius:8,marginBottom:16,
                background:msg.ok?'#EAF3DE':'#FCEBEB',color:msg.ok?'#27500A':'#A32D2D'}}>
                {msg.text}
              </div>
            )}
            ...
            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>setStep(2)}
                style={{flex:1,padding:'12px',fontSize:13,borderRadius:10,cursor:'pointer',
                  border:'0.5px solid #ddd',background:'#fff',color:'#666'}}>
                ← Volver al EV
              </button>
              <button onClick={confirmarYGuardar} disabled={saving}
                style={{flex:2,padding:'12px',fontSize:14,fontWeight:600,borderRadius:10,
                  border:'none',background:'#1a1a18',color:'#fff',cursor:'pointer'}}>
