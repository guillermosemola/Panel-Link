...
const SECTOR_CAMPOS = {
  ...
  comercial: ['precio_mercado_m2','ritmo_venta_m2','modo_cochera','cant_cocheras',
              'precio_cochera_usd','m2_cochera','precio_m2_cochera','ritmo_venta_cocheras'],
}
      ...
      if (vars && vars.length > 0) {
        ...
        vars.forEach(v => {
          Object.keys(DEFAULTS).forEach(k => {
            if (v[k] != null) merged[k] = typeof DEFAULTS[k] === 'number' ? +v[k] : v[k]
          })
        })
        setInputs(merged)
      }
    ...
    <div style={s.page}>
      <nav style={s.nav}>
        <div style={s.navL}>
          ...
          {/* Botón datos del proyecto — visible para finanzas y técnica */}
          {/* Botón certificados — todos los sectores */}
